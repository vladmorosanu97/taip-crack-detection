...
def HoughDetect_May_13(img, x, P, resolution=1):
    ...
    img_gray = cv2.GaussianBlur(img_gray, (9, 9), 0)  # Smoothes the image by convolving the with a (9,9) Gaussian filter
            ...
            for x1, y1, x2, y2 in lines[feature]:
                if (x1 - x2) != 0:  # Make sure lines are not collinear
                    ...
                    if abs(theta) > 0.1 and l_mag > 20:
                        if abs(theta) < 1.3:  # Eliminate short lines and lines that are close to being perfectly horizontal or vertical
    ...
    # Draw a  blue circle around the measurement point given by x axis of the intersection point and 1/3 rd of the image height.
