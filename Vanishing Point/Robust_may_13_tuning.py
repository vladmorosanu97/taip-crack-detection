...
def HoughDetect_May_13(img, x, P, resolution=1):
    ...
    # img : Input Image
    # x : Initial Vanishing Point Coordinates. It is of type Matrix (See Kalman_filter.py)
    ...
    img_gray = cv2.GaussianBlur(img_gray, (9, 9), 0)  # Smoothes the image by convolving it with a (9,9) Gaussian filter
    ...
    # The values of 'rho' and 'theta' represent the granularity of the detection scheme (
    # rho = 1 means check every pixel, theta =  1 degree means check for every 1 degree variation) and
    ...
    # The probabilistic Hough transform randomly samples sections of the image for lines
    # (this only works if the image is structurally rich).
    # The output is a list in which every element describes a line by its end points (x1,y1) and (x2,y2)
    # Threshold of 400 needs to be decreased with image resolution
    lines = cv2.HoughLinesP(grad_directions_8u, 1, np.pi/180, 400, minLineLength, maxLineGap)
        ...
        for feature in range(len(lines)):
            for x1, y1, x2, y2 in lines[feature]:
                if (x1 - x2) != 0:  # Make sure lines are not collinear
                    ...
                    # Extend the lines to the entire image and compute the intersection point
                    c2 = y1 - m2*x1
                    ...
                    if abs(theta) > 0.1 and l_mag > 20:
                        if abs(theta) < 1.3:  # delete short lines/lines close to being perfectly horizontal or vertical
